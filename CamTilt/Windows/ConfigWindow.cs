...
public class ConfigWindow : Window, IDisposable
{
    ...
    private void DrawSlider(string label, Func<float> getter, Action<float> setter, float min, float max, ImGuiSliderFlags flags = ImGuiSliderFlags.None, string Id = "")
    {
        var val = getter();
        if (Id != "") label += $"##{Id}";
        if (ImGui.SliderFloat(label, ref val, min, max, default, flags))
        {
            setter(val);
            Configuration.Save();
            OnConfigChanged?.Invoke();
    ...
    private void DrawDebugValues()
    {
        ...
        {
            ImGui.Separator();
            ImGui.SliderFloat("Y offset", ref rawAngle, -1, 1, default, ImGuiSliderFlags.NoInput);
            ImGui.SliderFloat("Pitch (0-1)", ref cleanAngle, 0, 1, default, ImGuiSliderFlags.NoInput);
            ImGui.SliderFloat("mapped tilt", ref mappedTilt, -.08f, .21f, default, ImGuiSliderFlags.NoInput);
            ImGui.Separator();
        }
    }
}
